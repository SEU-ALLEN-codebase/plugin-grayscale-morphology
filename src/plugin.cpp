...
bool NeuronImageRectifier::dofunc(const QString & func_name, const V3DPluginArgList & input, V3DPluginArgList & output, V3DPluginCallback2 & callback,  QWidget * parent)
{
    ...
    try
    {
        ...
        else
        {
            ...
            qDebug() << "Functions:";
            qDebug() << "";
            qDebug() << "high_pass_filter: make neuron fibers standing out and remove flare";
            ...
            qDebug() << "integrated: execute the above procedures sequentially";
            qDebug() << "";
            qDebug() << "Usage:";
            qDebug() << "";
            ...
            qDebug() << "vaa3d -x neuron_image_rectify -f enhancement -i in.tif -o out.tif -p enh_sigma 1,1,0.25 color_sigma 35 gain 5 cutoff 25 bilateral true fft true";
            qDebug() << "vaa3d -x neuron_image_rectify -f sparse_auto_thr -i in.tif -o out.marker -p fg_percentile 0.01 n_bin_float 256";
            qDebug() << "vaa3d -x neuron_image_rectify -f thr_hist_eq -i in.tif -o out.tif out.marker -p hist_eq_thr 0 dtype 1 n_bin_float 256";
            qDebug() << "vaa3d -x neuron_image_rectify -f meanshift_soma_refinement -i in.tif -o out.marker -p start_pos 0.5,0.5,0.5 win_radius 20,20,20 normalized_start true ms_sigma 3,3,3 gsdt true dt_threshold 0 test_count 50 z_thickness 1 connection_type 3";
