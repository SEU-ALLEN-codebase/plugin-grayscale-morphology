...
bool FFT_1d(complex_type* & data, V3DLONG size, const FFT_direction fft_direction)
{
    if(!isPower(size))
    {
        ...
        QSharedPointer<complex_type> data1_buffer(new complex_type[size_new], default_delete<complex_type[]>());
        if (data1_buffer.isNull())
            throw "Enhancement: Failed to allocate memory in fft.";
        auto&& data1 = data1_buffer.data();
...
void FFT_inplace(complex_type* &data, const V3DLONG sz[4], const FFT_direction fft_direction)
{
    V3DLONG n_cols  = sz[0];
    V3DLONG n_rows  = sz[1];
    V3DLONG n_depth = sz[2];

    complex_type* subarray;
    QSharedPointer<complex_type> subarray_buffer;

    subarray_buffer = QSharedPointer<complex_type>(new complex_type[n_cols], default_delete<complex_type[]>());
    if (subarray_buffer.isNull())
        throw "Enhancement: Memory allocation failed in fft.";
    subarray = subarray_buffer.data(); // for fft for rows: each column has n_cols elements
    ...
    subarray_buffer = QSharedPointer<complex_type>(new complex_type[n_rows], default_delete<complex_type[]>());
    if (subarray_buffer.isNull())
        throw "Enhancement: Memory allocation failed in fft.";
    subarray = subarray_buffer.data(); // for fft for columns: each row has n_rows elements
    ...
    // fft for depth
    subarray_buffer = QSharedPointer<complex_type>(new complex_type[n_depth], default_delete<complex_type[]>());
    if (subarray_buffer.isNull())
        throw "Enhancement: Memory allocation failed in fft.";
    subarray = subarray_buffer.data(); // each depth strip contains n_depth elements
