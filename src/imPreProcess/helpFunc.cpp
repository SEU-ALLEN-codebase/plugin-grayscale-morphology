    ...
    for(int k=0; k<kernelSZ[2]; ++k){
        ...
        for(int j=0; j<kernelSZ[1]; ++j){
            ...
            for(int i=0; i<kernelSZ[0]; ++i){
                x = pow(i-centerX,2);
                double g = 0;
                auto&& Sxx = spaceSigmaX*spaceSigmaX;
                auto&& Syy = spaceSigmaY*spaceSigmaY;
                auto&& Szz = spaceSigmaZ*spaceSigmaZ;
                if(spaceSigmaZ>0) g = exp(-x/(2*Sxx)-y/(2*Syy)-z/(2*Szz));
                else g = exp(-x/(2*Sxx)-y/(2*Syy));
                qDebug()<<g;
