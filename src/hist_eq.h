...
template <typename T>
void thr_hist_eq(T* buffer, V3DLONG size, V3DLONG n_bin, int thr)
{
...
template <typename T>
void thr_hist_eq2(T* buffer, V3DLONG size, V3DLONG n_bin, float thr_bin)
{
    T max = buffer[0], min = buffer[0];
    for (V3DLONG i = 0; i < size; ++i)
    {
    ...
    for (V3DLONG i = thr_bin + 1; i < n_bin; ++i)
        cumu[i] = cumu[i-1] + hist[i];
    auto&& range = max - hist[thr_bin];
    auto&& thr = thr_bin * bin_size + min;
    for (V3DLONG i = thr_bin; i < n_bin; ++i)
        cumu[i] = cumu[i] * range / cumu.back() + thr;
